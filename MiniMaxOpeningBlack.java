...
public class MiniMaxOpeningBlack {
    ...
    public static void main(String[] args) throws IOException {
        ...
        char[] tempB = HelperClass.swapColors(inputBoard);
        int minMaxEstimate = maxMinOpen(tempB, depth);
        opBoard = HelperClass.swapColors(opBoard);
    ...
    private static int minMaxOpen(char[] inputBoard, int depth) {
        if (depth == 0) {
            noOfStaticEstimates++;
            //System.out.println("1: " + HelperClass.printBoard(inputBoard) + " " + HelperClass.staticEstimateOpening(inputBoard));
            return HelperClass.staticEstimateOpening(inputBoard);
        } else {
